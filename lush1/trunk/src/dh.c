...
/***********************************************************************
 * $Id: dh.c,v 1.1 2002-07-02 19:56:38 leonb Exp $
 **********************************************************************/
             ...
             drec->op != DHT_NIL )
        {
          switch(drec->op) 
            ...
            case DHT_SRG:
              ...
              break;
            case DHT_LIST:
              /* Why is this different? [LEON] */
              drec->end = drec + drec->ndim + 2;
              drec++;
              for(i = drec->ndim - 1; i>=0; i-- ) 
                {
                  drec->end = drec+1;
                  drec++;
                }
  ...
  if (kdata && !kdata->lispdata.atclass)
    {
      ...
      if (drec->op != DHT_CLASS)
        error(NIL, "Malformed CLASSDOC: "
              "class description must start with class record", NIL);
      drec++;
      while (drec->op == DHT_NAME)
        {
...
/* ---------------------------------------------
   INITIALIZATION FUNCTIONSTORAGE_TO_DHT TABLES
   --------------------------------------------- */
