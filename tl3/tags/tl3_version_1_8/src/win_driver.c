...
$Id: win_driver.c,v 1.1.1.1 2002-04-16 17:37:39 leonb Exp $
***********************************************************************/
...
static CRITICAL_SECTION tl3event_cs;
static struct event *tl3event_buffer;
static int tl3event_evput, tl3event_evget;
...
/* tl3event_test -- called by wintl3 thread to test presence of events */

BOOL 
tl3event_test(void)
{
  BOOL ans = TRUE;
  EnterCriticalSection(&tl3event_cs);
  if (tl3event_evput == tl3event_evget)
    ans = FALSE;
  LeaveCriticalSection(&tl3event_cs);
  return ans;
}



/* buttonevents -- tl3 events for button messages */
...
/* keyevents -- tl3 events for keydown messages */
...
/* tl3event_add -- called by wintl3 thread to file new events */

BOOL 
tl3event_add(void *wid, int code, int arg, int x, int y)
{ 
  int i;
  struct event *ev;
  ...
  if (tl3event_evput!=tl3event_evget)
  {
    i = (tl3event_evput - 1) & EVBUFMASK;
    ev = &tl3event_buffer[i];
  ...
  /* Convert windows event into TLisp event */
  i = (tl3event_evput + 1) & EVBUFMASK;
  ev = &tl3event_buffer[tl3event_evput];
  if (translate_event(code, arg, x, y, ev) && i!=tl3event_evget)
  {
    ev->handler = (at*)wid;
    tl3event_evput = i;
    LeaveCriticalSection(&tl3event_cs);
...
/* transfer_events -- called to refill the TLisp event queue */

static void
transfer_events(void)
{
  /* Transfer events from temp queue to TLisp queue */
  EnterCriticalSection(&tl3event_cs);
  while (tl3event_evget != tl3event_evput)
  {
    int i;
    struct M_window *info = NULL;
    struct event *ev = &tl3event_buffer[tl3event_evget];
    /* search window */
    for (i=0; i<MAXWIN; i++)
    ...
    /* next event */
    tl3event_evget = (tl3event_evget + 1) & EVBUFMASK;
  }
  LeaveCriticalSection(&tl3event_cs);
}
...
kill_events(void *wid)
{
  ...
  EnterCriticalSection(&tl3event_cs);
  i = tl3event_evget;
  while (i != tl3event_evput)
  {
    if ((void*)tl3event_buffer[i].handler == wid)
      tl3event_buffer[i].handler = NIL;
    i = (i+1) & EVBUFMASK;
  }
  LeaveCriticalSection(&tl3event_cs);
}
...
wbm_end(struct window *linfo)
{
  ...
  if (--info->dbflag == 0)
  {
    ...
    if (! IsRectEmpty(&info->hitrect))
    {
      ...
      arg.status = MSG_OK;
      tl3send(TL3_UPDATEWIN, (TL3ARG*)&arg);
...
wbm_close(struct window *linfo)
{
  ...
  arg.wid = info->wid;
  arg.status = MSG_OK;
  tl3send(TL3_CLOSEWIN, (TL3ARG*)&arg);
  // clean events
...
wbm_xsize(struct window *linfo)
{
  ...
  arg.status = MSG_OK;
  arg.wid = info->wid;
  tl3send(TL3_QUERYWIN, (TL3ARG*)&arg);
...
wbm_hilite(struct window *linfo, int code, int x1, int y1, int x2, int y2)
{
  ...
  arg.h = y2;
  if (tl3port)
    tl3send(TL3_HILITEWIN, (TL3ARG*)&arg);
}
               ...
               char *name, BOOL toplevel)
{
  ...
  if (! tl3port)
    error("wbm_window","This function requires WinTL3",NIL);
  /* Perform RPC */
  arg.x = x;
  arg.y = y;
  arg.w = w;
  ...
  if (toplevel)
    tl3send(TL3_CREATETOPWIN, (TL3ARG*)&arg);
  else
    tl3send(TL3_CREATEWIN, (TL3ARG*)&arg);
...
DX(xwin_configure)
{
  ...
  if (w->gdriver == &wbm_driver)
  {
    ...
    if (arg.status)
      tl3send(TL3_CONFIGWIN, (TL3ARG*)&arg);
    else
      tl3send(TL3_QUERYWIN, (TL3ARG*)&arg);
               ...
               char *name)
{
  ...
  dinf.lpszDocName = "TL3 print request";
...
init_WINdriver(void)
{
  ...
  InitializeCriticalSection(&tl3event_cs);
  tl3event_buffer = (struct event*)tl_malloc( sizeof(struct event) * EVBUFSIZE );
  tl3event_evput = tl3event_evget = 0;
