...
start_lisp(int argc, char **argv)
{
  ...
  ifn (sigsetjmp(context->error_jump, 1)) 
    {
      ...
      if ((r = search_file(s,"|.dump")) && isdump(r))
	{
      ...
      else if ((r = search_file(s,"|.snc|.sn|.tlc|.tl")))
	{
  ...
  *line_buffer = 0;
  ...
  print_string("\n");
  do {
    /* CALLS THE TOPLEVEL PROCEDURE */
    q = apply(at_toplevel,NIL);
    UNLOCK(q);
  } while (isatty(fileno(stdin)) && ask("Really quit")==0);
  clean_up();
  exit(0);
}
