...
extern float **make_nr_idx2();
extern float *make_nr_idx1();
...
{
    int i,j, n_dim, *ind;
    flt **mat, *indx, *temp, res;
    
    n_dim = PL0_indx->dim[0];
    mat = make_nr_idx2(PL0_a);

    indx = make_nr_idx1(PL0_indx);
...
struct idx *PL0_c;
{
    ...
    mat = make_nr_idx2(PL0_a);
    indx = make_nr_idx1(PL0_b);
    b = make_nr_idx1(PL0_c);
...
struct idx *PL0_c;
{
    ...
    n_dim = PL0_a->dim[0];
    mat = make_nr_idx2(PL0_a);
    d = make_nr_idx1(PL0_b);
    e = make_nr_idx1(PL0_c);
...
flt L0_d;
{
    ...
    na = PL0_a->dim[1];
    mat = make_nr_idx2(PL0_a);
    w = make_nr_idx1(PL0_w);
    v = make_nr_idx2(PL0_v);
    ...
    struct idx *PL0_x;
{
    ...
    m = PL0_u->dim[0];
    n = PL0_u->dim[1];
    u = make_nr_idx2(PL0_u);
    w = make_nr_idx1(PL0_w);
    v = make_nr_idx2(PL0_v);
    b = make_nr_idx1(PL0_b);
    x = make_nr_idx1(PL0_x);
...
/******************************************************************************
 *  
 *   (nr-simplx <a>:idx2-flt
 *              <m1><m2><m3>:int
 *              <izrov><iposv>:idx1-int )
 *   returns <icase>:int
 *
 *   Runs the simplex algorithm.
 *   See Numerical Recipes page 339
 *
 *****************************************************************************/


int 
C_nr_simplx (L1_a, L1_m1, L1_m2, L1_m3, L1_izrov, L1_iposv)
struct idx *L1_a;
int L1_m1;
int L1_m2;
int L1_m3;
...
struct idx *L1_iposv;
{
    ...
    mp2 = L1_a->dim[0];
    np1 = L1_a->dim[1];
    if (L1_m1 + L1_m2 + L1_m3 != mp2 - 2)
      nrerror("Number of constraints does not match size of matrix A");
    if (L1_izrov->dim[0] != np1 - 1)
      nrerror("IZROV should have size N (number of variables)");
    if (L1_iposv->dim[0] != mp2 - 2)
      nrerror("IPOSV should have size M (number of constraints)");
    a = make_nr_idx2(L1_a);
    izrov = make_nr_idx1_int(L1_izrov);
    iposv = make_nr_idx1_int(L1_iposv);
    simplx(a, mp2-2, np1-1, L1_m1, L1_m2, L1_m3, &icase, izrov, iposv);
    free(a + 1);
    return icase;
}
...
{
  dharg ret;
  ret.dh_int = C_nr_simplx (a[1].dh_idx_ptr, 
                         a[2].dh_int, a[3].dh_int, a[4].dh_int, 
                         a[5].dh_idx_ptr, a[6].dh_idx_ptr);
  return ret;
}
#endif
 ...
 */
#ifndef NOLISP
DHDOC (K_nr_simplx, X_nr_simplx, "C_nr_simplx", NULL, NULL)
{
    DH_FUNC (6),
    DH_IDX (DHT_READ, 2),
    DH_SRG (DHT_READ),
    ...
    init_nr_some ()
{
    ...
    dh_define ("nr-simplx", &K_nr_simplx);
}

int majver_nr_some = 3;
int minver_nr_some = 2;
